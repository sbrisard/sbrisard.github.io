...
The following code snippet ([download](./files/tif2hdf5.py)) converts the binary
`*.tif` images into a single `*.hdf5` file. It uses the
...
The above code snippet ([download](./files/connected_components.py)) detects 117
connected components, and produces the following colored image (one color per
label), see Fig. @fig-02. Unsurprisingly, all grains are connected on this
slice, and we have produced a very poor segmentation indeed!
...
below does ([download](./files/watershed-distance_local_max.py)). It draws
heavily on the example from the
