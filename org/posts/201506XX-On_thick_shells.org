...
This is the first surprise in this series: the stress couples are /not/ null, and the stress resultant differ slightly from the well-known formula \(N=pR/2\) ([[http://en.wikipedia.org/wiki/Pressure_vessel#Stress_in_thin-walled_pressure_vessels][Wikipedia]]).
...
So what's wrong with the above analysis? To dig deeper into this problem, we will use its full 3D solution to compute /exact/ values of the stress resultants and couples... which will lead to other exciting findings!
