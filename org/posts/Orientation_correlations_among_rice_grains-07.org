...
\end{equation}
where the integral is carried out over the grain, and $X_i$ denotes the $i$-th coordinate of the grain's center of gravity

\begin{equation}
\label{eq:2}X_i = \frac1V\int_\text{Grain}x_i\,\diff x_1\,\diff x_2\,\diff x_3
\end{equation}

($V$: volume of the grain). In intrinsic form, Eq. \eqref{eq:1} reads

\begin{equation}
\label{eq:3}\tensii J=\int_\text{Grain}\left(\vec x-\vec X\right)\otimes\left(\vec x-\vec X\right)\diff x_1\,\diff x_2\,\diff x_3.
\end{equation}

The above defined tensor of second moments is related to the [[https://en.wikipedia.org/wiki/Moment_of_inertia#The_inertia_tensor][inertia tensor]] $\tensii I$
\begin{equation}
...
Being symmetric, the tensor $\tensii J$ of second moments is diagonalizable, and we compute its eigenvalues $J_\mathrm{I}\geq J_\mathrm{II}\geq J_\mathrm{III}$, and the associated eigenvectors $\vec v_\mathrm{I}$, $\vec v_\mathrm{II}$ and $\vec v_\mathrm{III}$:

\begin{equation}
\tensii J\cdot\vec v_\alpha=J_\alpha\vec v_\alpha,
\end{equation}

where $\alpha=\mathrm{I}, \mathrm{II}, \mathrm{III}$. In the present post, we define the orientation of the grain as the eigenvector $\vec v_\mathrm{I}$ associated to the /largest/ eigenvalue $J_\mathrm{I}$.

equivalent ellipsoid, with same volume and inertia. More precisely,
...
It is observed that, while the length of most grains is about 2×27=54 pixels (about 6.5 mm), a significant number of grains are very small. There are two possible explanations for this
