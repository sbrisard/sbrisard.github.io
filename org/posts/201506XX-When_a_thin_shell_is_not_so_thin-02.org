...
To define a subregion of the shell, we first draw a closed path $\Gamma$ on the base surface. As the current point moves along this path, the normal to the base surface sweeps a new surface, which will be the /lateral surface/ of the subregion. It should be noted that, by construction, the lateral surface is a ruled surface.
