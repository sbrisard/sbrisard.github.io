...
In the [[sb-blog-prev:][previous instalment]] of this series, we showed that the total energy of
the periodic-plus-smooth decomposition of an image ([[moisan2011:][Moisan, 2011]]) is a
quadratic form which can be cast in the standard form

\begin{equation*}
E(p, s)=E_\mathrm p(p)+E'_\mathrm s(s)
\quad\text{with}\quad
...
In theory, the linear operators $Q_1$ and $Q_2$ are matrices, and could be
implemented as such. However, for $m\times n$ images, the matrices would be
unnecessarily large: $mn\times mn$! We will therefore adopt here a
[[https://en.wikipedia.org/wiki/Matrix-free_methods][matrix-free approach]]. The remainder of this post is organized as follows. We
will first discuss [[linear_operators][linear operators]] in the =scipy.sparse.linalg= sense. Then,
we will [[Q1_implementation][implement the $Q_1$ operator]]. Finally, we will
[[Q2_implementation][implement the $Q'_2$ operator]] (or rather, the $Q_2$ operator).

* Matrix-free linear operators <<linear_operators>>

The $Q_1$ and $Q'_2$ linear operators will be implemented as instances of
...
Essentially, what this means is that $Q_1$ and $Q'_2$ are seen as /functions/
...
solving linear systems that involve $Q_1$ and $Q'_2$ can then be done by means
of [[https://en.wikipedia.org/wiki/Iterative_method#Linear_systems][iterative linear solvers]], such as the [[https://en.wikipedia.org/wiki/Conjugate_gradient_method][conjugate gradient method]]. Interesting
references on this topic are the book by Y. Saad:
[[https://doi.org/10.1137/1.9780898718003][Iterative Methods for Sparse Linear Systems]] and the book by Barrett /et al./:
[[http://epubs.siam.org/doi/book/10.1137/1.9781611971538][Templates for the Solution of Linear Systems: Building Blocks for Iterative Methods]]
(also freely available [[http://www.netlib.org/templates/][here]]).
...
that defines linear operators that operate on images. These operators store the
shape of the image.
...
This class is to be used to implement the operators $Q_1$ and $Q'_2$.
...
class OperatorQ1(ImageLinearOperator):
    ...
    where ' is the transpose and F(s) is the function to be minimized
    with respect to the smooth component s. F is defined by Eq. (8)
...
* Implementation of the $Q'_2$ operator <<Q2_implementation>>

The $Q_2$ operator was defined in the [[sb-blog-prev][previous post]] as the following sum:
$Q_2=Q_2^\mathrm h+Q_2^\mathrm v+Q_2^\mathrm m$, with
