...
In the [[sb-blog-03:][previous instalment]] of this series, we introduced the linear operators
...
follows. We will first discuss [[#operators][linear operators]] in the =scipy.sparse.linalg=
sense. Then, we will implement [[#Q1][the $Q_1$ operator]] and [[#Q][the $Q$ operator]].
Finally, we will [[#testing][test our implementation]].

This post is the fourth instalment of a series in {{{sb-blog-num-posts}}}
parts:
...
Linear Systems]] and the book by Barrett /et al./: [[http://epubs.siam.org/doi/book/10.1137/1.9781611971538][Templates for the Solution of Linear Systems: Building Blocks for Iterative Methods]] (also freely available
...
The $Q_1$ operator was defined in the [[sb-blog-03a:#Q1][previous post]] as the following sum:
...
The $Q$ operator was defined in the [[sb-blog-03a:#Q][previous post]] as the following sum:
...
the kernel, which is of the same =dtype= as the images to which the operator
is to be applied. This allows for /exact/ computations on integer images.
...
where the energy $E$ was implemented in [[sb-blog-02a:#implementation][part 2]] of this series.

#+BEGIN_SRC ipython :exports results :results output code
  from test_moisan2011 import img_shapes, test_OperatorQ1, test_OperatorQ

  print(inspect.getsource(img_shapes))
  print()
  print(inspect.getsource(test_OperatorQ))
  print()
  print(inspect.getsource(test_OperatorQ1))
#+END_SRC
...
The above code snippet calls for several remarks
  ...
  1. We used the great [[https://pytest.org/][pytest]] library to write the unit tests: this allows for
     parameterized tests.
...
of an image. This will be done in the [[sb-blog-05:][next instalment]] of this series
