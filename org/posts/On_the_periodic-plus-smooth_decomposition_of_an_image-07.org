...
#+LINK: sb-blog-prev ./On_the_periodic-plus-smooth_decomposition_of_an_image-06.org
#+LINK: sb-blog-next ./On_the_periodic-plus-smooth_decomposition_of_an_image-09.org
...
In the [[sb-blog-prev:][previous instalment]] of this series, we implemented Moisan's ([[moisan2011:][2011]])
...
In the [[sb-blog-prev:][previous instalment]] of this series, we showed that the DFT $\hat s$ of
...
implementation of =_per= in the previous post).
...
=numpy.fft.rfft2= function rather than =numpy.fft.fft2=. This what is done
below.
...
This is nearly the end of the story. We now have good implementation of
Moisan's algorithm. We have optimized its implementation, but the code did not
lose in clarity.

In the [[sb-blog-next:][next instalment]] of this series, I will put everything together in a
python module.
