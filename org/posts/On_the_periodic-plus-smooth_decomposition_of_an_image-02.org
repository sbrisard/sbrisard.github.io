...
we will define /direct/ and /indirect/ neighbors. By definition, direct
neighbors are adjacent to the pixel under consideration. Conversely, indirect
neighbors are periodic images of these pixels that fall outside the
image. Figure [[fig:direct_and_indirect_neighbors]] illustrates directe and indirect
neighbors. It results from these definitions that
...
Now, we need to quantify how periodic is the periodic component $p$, and how
smooth is the smooth component $s$. More precisely, we will define to energy
functions: $E_\mathrm{p}$ and $E_\mathrm{s}$ that penalize lack of periodicity
and smoothness, respectively. More precisely, $E_\mathrm{p}$
