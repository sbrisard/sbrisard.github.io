...
Now, we need to quantify how periodic is $p$, and
how smooth is $s$. To do so, we will define two
energy functions: $E_\mathrm p$
and $E_\mathrm s$ that penalize
lack of periodicity and smoothness, respectively. More precisely,
$E_\mathrm p$
(resp. $E_\mathrm s$) is greater
for less periodic (resp. less smooth) $p$
(resp. $s$). The periodic-plus-smooth decomposition
of an image $u$ is then defined as the pair of
images $(p, s)$ that
minimize the total energy
$E(p, s)=E_\mathrm p(p)+E_\mathrm s(s)$
under the constraints $u=p+s$
and $\operatorname{mean}(s)=0$. The
