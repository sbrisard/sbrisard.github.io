...
In the present post, neighbor pixels refer to the [[https://en.wikipedia.org/wiki/Pixel_connectivity#4-connected][4-connectivity]]. Now, we need
to introduce measures of periodicity and smoothness.
...
image boundaries (see Figure [[fig:2]]). This leads to the following expression
...
#+CAPTION: Periodic neighbors (blue) of a specific pixel (orange) to be
#+CAPTION: considered for the evaluation of the measure of periodicity, for
#+CAPTION: corner pixels (top row), and off-corner pixels (bottom row).
#+NAME: fig:2
#+ATTR_HTML: :width 90%
[[sb-blog-data-dir:periodic_neighbors.svg]]

Now, smoothness is evaluated by computing the sum of squares of all jumps
between “true” (adjacent) neighbors: periodic images are excluded (see Figure
[[fig:3]]). This leads to the following expression
...
#+CAPTION: Adacent neighbors (blue) of a specific pixel (orange) to be
#+CAPTION: considered for the evaluation of the measure of smoothness, for
#+CAPTION: corner pixels (top row), off-corner, boundary pixels (middle row),
#+CAPTION: and off-boundary pixels (bottom row).
#+NAME: fig:3
#+ATTR_HTML: :width 90%
[[sb-blog-data-dir:adjacent_neighbors.svg]]
