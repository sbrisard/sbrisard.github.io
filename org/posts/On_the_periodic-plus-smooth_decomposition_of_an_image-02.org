...
that we will eventually solve. To do so, it will prove convenient to introduce
the following functionals

\begin{equation*}
F_1(s, u)=E_\mathrm p(u-s)
\quad\text{and}\quad
F_2(s)=E_\mathrm s(s)+(\operatorname{mean}s)^2.
\end{equation*}
...
[[sb-blog-next:][next instalment]] of this series, we will however temporarily ignore this result,
