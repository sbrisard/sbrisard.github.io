...
operator $A_\mathrm h$ that maps $p$ onto $h$. This operator is obviously
symmetric. Indeed, $p$ and $q$ being two $m\times n$ images,
\begin{equation*}
\begin{aligned}
(A\cdot p)\cdot q&=\sum_{i=0}^{m-1}\sum_{j=0}^{n-1}(A\cdot p)_{ij}q_{ij}
