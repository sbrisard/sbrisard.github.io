...
\begin{equation*}
(B_\mathrm h\cdot v)_{i,j}=
\begin{cases}
2(v_{0, j}-v_{1, j})              & \text{if }i=0,\\
2(v_{m-1, j}-v_{m-2, j})          & \text{if }i=m-1,\\
2(2v_{i, j}-v_{i-1, j}-v_{i+1,j}) & \text{otherwise}.
\end{cases}
\end{equation*}

From the left-hand side of Eq. \eqref{eq:4}, it is readily found that
$B_\mathrm h$ is a symmetric, positive operator. Furthermore, the first term of Eq. ????? reads
\begin{equation*}
2\sum_{i=0}^{m-2}\sum_{j=0}^{n-1}(s_{i+1, j}-s_{i, j})^2=s\cdot B_\mathrm h\cdot s.
\end{equation*}
