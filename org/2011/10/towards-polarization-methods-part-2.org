...
In [[file:../../2011/10/towards-polarization-methods-part-1.org][part 1]] of this series, we introduced the Lippmann--Schwinger equation. Direct discretization of this equation can lead to efficient schemes for the numerical solution of the elementary problem of micromechanics. However, my experience shows that taking first the variational form of this equation, and following standard Garlerkin discretization approaches can lead to much more robust and efficient schemes. In this instalment, we will introduce the so-called variational "principle" of [[file:~/Documents/blog/org/p/references.html::HASH1962][Hashin and Shtrikman (1962)]], @@html:<!-- more -->@@ which is nothing but the variational form of the Lippmann-Schwinger equation. The present exposition follows quite closely the approach of [[file:~/Documents/blog/org/p/references.html::WILL1977][Willis (1977)]].

In the [[file:../../2011/10/towards-polarization-methods-part-1.org][previous instalment]], I proved that solving the following elasticity problem on $\domain$
...
the right-hand side being zero if the polarization field $\polarization$ is the solution to the Lippmann-Schwinger equation \eqref{eq:5}. Therefore, the first variation of $\mathcal H$ at $\polarization$ is null; in other words, $\polarization$ is a /critical point/ of $\mathcal H$. This result is usually referred to as the variational principle of [[file:../../p/references.org::<<HASH1962>>][Hashin and Shtrikman (1962)]]. It is the fundamental result at the basis of all numerical methods presented in this blog.

So, it is time to sum up. We proved in this instalment that solving the boundary value problem \eqref{eq:1} -- \eqref{eq:4} is actually equivalent to finding the critical point of $\mathcal H$ defined by Eq. \eqref{eq:8}. It can be proved that this critical point is indeed unique. In the [[file:./towards-polarization-methods-part-3.org][next instalment]] of this series, I will introduce the /energy principle/ of [[file:../../p/references.org::<<HASH1962>>][Hashin and Shtrikman (1962)]]. In other words, we will explore under which circumstances this critical point is an /extremum/ of $\mathcal H$.

* History

  - <2011-10-31 Mon> Initial version.
  - <2014-01-11 Sat> Used MathJax's features for automatic numbering of equations.
