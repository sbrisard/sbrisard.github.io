...
\label{eq:2}
\stress(\vec x)&=\tens{C}:\strain(\vec x)+\tens{\varpi}(\vec x)&(\vec x\in\reals^d)\\
\label{eq:3}
...
\begin{align*}
\hat\stress_b&=i\left(\tens{C}\cdot\vec k_b\right)\cdot\hat\vec{u}_b+\hat\tens{\varpi}_b&(b\in\integers^d).
\end{align*}

The equilibrium equation \eqref{eq:1} finally gives

\begin{align*}
\vec 0&=\vec k_b\cdot\hat\stress_b=i\left(\vec k_b\cdot\tens{C}\cdot\vec k_b\right)\cdot\hat\vec{u}_b+\vec k_b\cdot\hat\tens{\varpi}_b&(b\in\integers^d).
\end{align*}
...
\begin{align*}
\hat\strain_b={}&-\frac12\vec k_b\otimes\left(\left(\vec k_b\cdot\tens{C}\cdot\vec k_b\right)^{-1}\cdot\left(\vec k_b\cdot\hat\tens{\varpi}_b\right)\right)\\
...
\begin{align*}
\hat\strain_b={}&-\frac12\left(\vec k_b\otimes\vec k_b\right)\cdot\hat\tens{\varpi}\cdot\transpinv{\left(\vec k_b\cdot\tens{C}\cdot\vec k_b\right)}\\
...
\begin{align*}
\hat\strain_b={}&-\frac12\left(\left(\vec k_b\otimes\vec k_b\right)\boxtimes\left(\vec k_b\cdot\tens{C}\cdot\vec k_b\right)^{-1}\right):\hat\tens{\varpi}_b\\
