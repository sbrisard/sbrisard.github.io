...
\label{eq:2}
\stress(\vec x)&=\tens{C}:\tens{\varepsilon}(\vec x)+\tens{\varpi}(\vec x)&(\vec x\in\reals^d)\\
\label{eq:3}
...
where $b=(b_1,\ldots,b_d)\in\integers^d$ is a multi-index, and $\Omega=(0,L_1)\times\cdots\times(0,L_d)$ denotes the unit-cell. Similarly, the strain $\tens{\varepsilon}$, stress $\stress$ and prestress $\tens{\varpi}$ are expanded in Fourier series
