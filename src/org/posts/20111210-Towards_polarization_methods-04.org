...
where the second term accounts for the fact that the reference material is homogeneous. The above expression can be further simplified. Indeed, $\volavg\strain=\overline{\tens{\varepsilon}}$ (this is the very definition of the macroscopic strain $\overline{\tens{\varepsilon}}$). Furthermore, $\volavg{\tens{C}:\strain} = \volavg{\stress} = \overline{\tens{\sigma}}$, where $\overline{\tens{\sigma}}$ is the macroscopic stress in the composite. This macroscopic stress is in turn related to the macroscopic strain through the homogenized constitutive law, which reads in linear elasticity
