...
\begin{equation}
\label{eq:5}
\mathcal H(\tens{\tau})=\frac12\overline{\tens{\varepsilon}}:\volavg{\left(\tens{C}-\tens{C}_0\right):\strain}=\frac12\overline{\tens{\varepsilon}}:\volavg{\tens{C}:\strain}-\frac12\overline{\tens{\varepsilon}}:\tens{C}_0:\volavg{\strain},
\end{equation}

where the second term accounts for the fact that the reference material is homogeneous. The above expression can be further simplified. Indeed, $\volavg\strain=\overline{\tens{\varepsilon}}$ (this is the very definition of the macroscopic strain $\overline{\tens{\varepsilon}}$). Furthermore, $\volavg{\tens{C}:\strain} = \volavg{\stress} = \Stress$, where $\Stress$ is the macroscopic stress in the composite. This macroscopic stress is in turn related to the macroscopic strain through the homogenized constitutive law, which reads in linear elasticity

\begin{equation}
\label{eq:6}
\Stress=\tens{C}^\eff:\overline{\tens{\varepsilon}},
\end{equation}
