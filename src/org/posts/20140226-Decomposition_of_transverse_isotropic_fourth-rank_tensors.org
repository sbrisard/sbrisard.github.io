...
As a preliminary note, it should be observed that any fourth-rank tensor $\tensiv A$ can be viewed as a endomorphism over the space of second-rank tensors, since $\tens x\mapsto\tensiv A:\tens x$ is a linear mapping (the convention adopted in this blog for the double dot product has been specified in a [[file:./20140219-On_the_double_dot_product.org][previous post]]). Therefore, the fourth-rank tensor $\tens a\boxtimes\tens b$ ($\tens a, \tens b$: second-rank tensors) can be defined by how it operates on second-rank tensors $\tens x$
