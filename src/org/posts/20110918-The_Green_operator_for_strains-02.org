...
Let $\tens{\varepsilon}=-\tens{\Gamma}_0\circledast\tens{\varpi}$, and $\stress=\tens{C}_0:\tens{\varepsilon}+\tens{\varpi}$. By definition of $\tens{\Gamma}_0$, $\tens{\varepsilon}$ is geometrically compatible and $\stress$ is self-equilibrated: $\nabla\cdot\stress=\tens 0$. From the previous property, we have $\tens{\Gamma}_0\circledast\stress=\tens0$. In other words
\begin{equation*}
...
To close this instalment, we again consider a prestress $\tens{\varpi}$, and the associated strain $\tens{\varepsilon}=-\tens{\Gamma}_0\circledast\tens{\varpi}$. We seek the value of $\volavg\tens{\varepsilon}$, where overlined quantities denote /volume averages/
\begin{equation*}
