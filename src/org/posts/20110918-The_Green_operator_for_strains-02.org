...
Obviously, the solution to Eqs. \eqref{eq:1}, \eqref{eq:2} and \eqref{eq:3} is $\vec{u}=\vec 0$ (and $\strain=\nabla^\text{s}\vec{u}=\tens0$). Since $\tens{\Gamma}_0\circledast\tens{\varpi}=-\strain$, it is finally found that $\tens{\Gamma}_0\circledast\tens{\varpi}=\tens 0$. To sum up, we have proved the following property
