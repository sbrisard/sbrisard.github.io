...
\begin{equation*}
\begin{aligned}[b]
\hat{v}_{\alpha\beta}^\mathrm h&=\sum_{i=0}^{m-1}\sum_{j=0}^{n-1}v_{ij}
...
This is the end of the story. We now have a good implementation of Moisan's
algorithm. We have optimized its implementation, but the code did not lose in
clarity.

If you are interested by my implementation of Moisan's algorithm, go to the
[[https://github.com/sbrisard/moisan2011][GitHub]] repository of the =moisan2011= Python module!
