...
To compute the elastic acoustic tensor, we therefore need to find the expressions of $\vec n\cdot\sphprojiv\cdot\vec n$ and $\vec n\cdot\tens{K}\cdot\vec n$, for any unit vector $\vec n$. We start with the computation of $\vec n\cdot\tens{I}\cdot\vec n$, where $\tens{I}$ denotes the fourth-rank identity tensor. We have
...
\begin{equation}
\label{eq:4}
\vec n\cdot\tens{I}\cdot\vec n=\tensii p+\frac12\tensii q.
\end{equation}
...
Finally, combining identity $\tens{K}=\tens{I}-\sphprojiv$ with Eqs. \eqref{eq:4} and \eqref{eq:5}
