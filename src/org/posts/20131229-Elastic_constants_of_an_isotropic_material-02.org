...
Combination of Eqs. \eqref{eq:1} and \eqref{eq:2} for $(i, j)=(3, 1)$ and $(i, j)=(2, 3)$, first leads to $\tens\sigma_{31}=\tens\sigma_{23}=0$. Then, for $(i, j) = (3, 3)$, $\tens\sigma_{33}=\nu\tens\sigma_{\gamma\gamma}$. The 3D trace of $\tens\sigma$ can therefore be expressed as a function of its 2D trace
\begin{equation*}
    \tens\sigma_{kk}=\left(1+\nu\right)\tens\sigma_{\gamma\gamma}.
\end{equation*}

Substituting in Eq. \eqref{eq:2}
\begin{equation}
    \label{eq:3}
    \varepsilon_{\alpha\beta}=\frac{1+\nu}E\left(\tens\sigma_{\alpha\beta}-\nu\tens\sigma_{\gamma\gamma}\delta_{\alpha\beta}\right),
\end{equation}
...
\begin{equation}
    \label{eq:4}
    \varepsilon_{\alpha\beta}=\frac1{2\shearmod}\left(\tens\sigma_{\alpha\beta}-\nu\tens\sigma_{\gamma\gamma}\delta_{\alpha\beta}\right).
\end{equation}

* Bulk and shear moduli

For (2D) isotropic stresses, $\tens\sigma_{\alpha\beta}=-p\delta_{\alpha\beta}$, Eq. \eqref{eq:4} reads
\begin{equation*}
...
\begin{equation*}
    \varepsilon_\text s=-\frac p{\bulkmod_\text{2D}},
\end{equation*}
with
\begin{equation}
    \label{eq:5}
