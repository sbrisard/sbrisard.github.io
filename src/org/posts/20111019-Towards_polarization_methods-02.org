...
In other words, the boundary value problem \eqref{eq:1} -- \eqref{eq:4} is replaced by a single integral equation \eqref{eq:5}. Classically, both sides of this equation are now multiplied by an arbitrary test field $\prestress$. After integration over the domain $\Omega$ we find for all test fields $\prestress$

\begin{equation}
\label{eq:6}
\volavg{\prestress:\left(\stiffness-\stiffness_0\right)^{-1}:\tens{\tau}}+\volavg{\prestress:\left(\tens{\Gamma}_0\circledast\tens{\tau}\right)}=\Strain:\volavg{\prestress}.
\end{equation}

This is a standard variational problem. Indeed, introducing the bilinear form $a$ and the linear form $\ell$

\begin{align*}
a(\tens{\tau}, \prestress)&=\volavg{\prestress:\left(\stiffness-\stiffness_0\right)^{-1}:\tens{\tau}}+\volavg{\prestress:\left(\tens{\Gamma}_0\circledast\tens{\tau}\right)},\\
...
\begin{equation}
\label{eq:7}
\text{Find }\tens{\tau}\,\text{such as }a(\tens{\tau}, \prestress)=\ell(\prestress)\,\text{for all }\prestress.
\end{equation}

It should be noted that in order to be well-posed, the spaces to which $\tens{\tau}$ and $\prestress$ belong should be specified. This can be done, and is actually the topic of a publication being currently reviewed.
...
\begin{equation}
\mathcal H(\tens{\tau}+\prestress)-\mathcal H(\tens{\tau})\simeq\ell(\prestress)-a(\tens{\tau}, \prestress),
\end{equation}
