...
\begin{equation}
\label{eq:5}
\left(\stiffness-\stiffness_0\right)^{-1}:\polarization+\left(\tens{\Gamma}_0\circledast\polarization\right)=\Strain.
\end{equation}
...
\begin{equation}
\label{eq:6}
\volavg{\prestress:\left(\stiffness-\stiffness_0\right)^{-1}:\polarization}+\volavg{\prestress:\left(\tens{\Gamma}_0\circledast\polarization\right)}=\Strain:\volavg{\prestress}.
\end{equation}

This is a standard variational problem. Indeed, introducing the bilinear form $a$ and the linear form $\ell$

\begin{align*}
a(\polarization, \prestress)&=\volavg{\prestress:\left(\stiffness-\stiffness_0\right)^{-1}:\polarization}+\volavg{\prestress:\left(\tens{\Gamma}_0\circledast\polarization\right)},\\
...
the right-hand side being zero if the polarization field $\polarization$ is the solution to the Lippmann-Schwinger equation \eqref{eq:5}. Therefore, the first variation of $\mathcal H$ at $\polarization$ is null; in other words, $\polarization$ is a /critical point/ of $\mathcal H$. This result is usually referred to as the variational principle of [[hash1962][Hashin and Shtrikman (1962)]]. It is the fundamental result at the basis of all numerical methods presented in this blog.
