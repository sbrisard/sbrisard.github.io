...
\begin{align}
    ...
    \label{eq:2}
    \stress(\vec x)&=\stiffness_0:\strain(\vec x)+\prestress(\vec x) && (\vec x\in\Omega),\\
    \label{eq:3}
...
Eq. \eqref{eq:1} expresses static equilibrium ($\stress$: local stress tensor); Eq. \eqref{eq:2} is the constitutive law of the homogeneous, prestressed material ($\strain$: local strain tensor). Eq. \eqref{eq:3} expresses the fact that $\strain$ is /geometrically compatible/: it is the symmetric part of the gradient of the displacement $\vec{u}$. Finally, Eq. \eqref{eq:4} specifies the boundary conditions.

So, this is the problem we would like to solve. From the mathematical theory of elasticity, we /know/ that it has a /unique/ solution $\vec{u}$. Again, displacements are not that interesting from the perspective of micromechanics; what we really want to determine is the local strain $\strain$. In order to express the solution to Eqs. \eqref{eq:1} – \eqref{eq:4} in a convenient form, it is essential to note that it is linear. The superposition theorem will therefore be invoked. As a first step, we assume that the prestress $\prestress$ has the simple following form
...
where $\vec y\in\Omega$ is a fixed point, $\delta$ is the Dirac function, and $\Prestress$ is a constant tensor. Because of linearity of Eqs. \eqref{eq:1} – \eqref{eq:4}, the strain $\strain(\vec x)$ at point $\vec x\in\Omega$ depends linearly on the only loading parameter, $\Prestress$. By definition, the linear operator mapping $\Prestress$ onto $\strain(\vec x)$ will be called the /Green operator for strains/, and denoted $\tens{\Gamma}_0$. Of course, this operator depends on the /observation/ point $\vec x$; it also depends on the /source/ point $\vec y$ [where the point prestress \eqref{eq:5} is actually applied]. In other words

\begin{equation}
\label{eq:6}
\strain(\vec x)=-\tens{\Gamma}_0(\vec x, \vec y):\Prestress,
\end{equation}
...
\begin{equation*}
\prestress(\vec x)=\int_{\vec y\in\Omega}\delta(\vec x-\vec y)\prestress(\vec y)\dV{\vec y}.
\end{equation*}

Application of the superposition theorem allows us to recover the general expression of the solution from Eq. \eqref{eq:6}

\begin{equation*}
\strain(\vec x) = -\int_{\vec y\in\Omega}\tens{\Gamma}_0(\vec x, \vec y):\prestress(\vec y)\dV{\vec y}.
\end{equation*}

The integral in the right-hand side is a kind of modified convolution product, which will be denoted '$\circledast$'

\begin{equation*}
\left(\tens{\Gamma}_0\circledast\prestress\right)(\vec x) = \int_{\vec y\in\Omega}\tens{\Gamma}_0(\vec x, \vec y):\prestress(\vec y)\dV{\vec y},
\end{equation*}

and the solution to Eqs. \eqref{eq:1} – \eqref{eq:4} reads

\begin{equation*}
\strain(\vec x) = -\left(\tens{\Gamma}_0\circledast\prestress\right)(\vec x).
\end{equation*}
...
\begin{equation}
\label{eq:8}
\strain(\vec x) = \Strain-\left(\tens{\Gamma}_0\circledast\prestress\right)(\vec x).
\end{equation}
