...
\begin{equation*}
\strain(\vec x) = -\int_{\vec y\in\Omega}\greeniv_0(\vec x, \vec y):\prestress(\vec y)\dV{\vec y}.
\end{equation*}

The integral in the right-hand side is a kind of modified convolution product, which will be denoted '$\circledast$'

\begin{equation*}
\left(\greeniv_0\circledast\prestress\right)(\vec x) = \int_{\vec y\in\Omega}\greeniv_0(\vec x, \vec y):\prestress(\vec y)\dV{\vec y},
\end{equation*}
