...
\begin{align}
    ...
    \label{eq:2}
    \stress(\vec x)&=\stiffness_0:\strain(\vec x)+\tens{\varpi}(\vec x) && (\vec x\in\Omega),\\
    \label{eq:3}
...
where $\vec y\in\Omega$ is a fixed point, $\delta$ is the Dirac function, and $\tens{\varpi}$ is a constant tensor. Because of linearity of Eqs. \eqref{eq:1} – \eqref{eq:4}, the strain $\strain(\vec x)$ at point $\vec x\in\Omega$ depends linearly on the only loading parameter, $\tens{\varpi}$. By definition, the linear operator mapping $\tens{\varpi}$ onto $\strain(\vec x)$ will be called the /Green operator for strains/, and denoted $\tens{\Gamma}_0$. Of course, this operator depends on the /observation/ point $\vec x$; it also depends on the /source/ point $\vec y$ [where the point prestress \eqref{eq:5} is actually applied]. In other words

\begin{equation}
\label{eq:6}
\strain(\vec x)=-\tens{\Gamma}_0(\vec x, \vec y):\tens{\varpi},
\end{equation}
...
\begin{equation*}
\strain(\vec x) = -\int_{\vec y\in\Omega}\tens{\Gamma}_0(\vec x, \vec y):\tens{\varpi}(\vec y)\dV{\vec y}.
\end{equation*}
...
\begin{equation*}
\strain(\vec x) = -\left(\tens{\Gamma}_0\circledast\tens{\varpi}\right)(\vec x).
\end{equation*}
...
\begin{equation}
\label{eq:8}
\strain(\vec x) = \Strain-\left(\tens{\Gamma}_0\circledast\tens{\varpi}\right)(\vec x).
\end{equation}
