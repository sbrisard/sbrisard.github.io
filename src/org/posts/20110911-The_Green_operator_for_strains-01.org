...
\begin{align}
    \label{eq:1}
    \nabla_\vec x\cdot\tens\sigma&=\vec{0} && (\vec x\in\domain),\\
    \label{eq:2}
    \tens\sigma(\vec x)&=\stiffness_0:\strain(\vec x)+\prestress(\vec x) && (\vec x\in\domain),\\
    \label{eq:3}
...
Eq. \eqref{eq:1} expresses static equilibrium ($\tens\sigma$: local stress tensor); Eq. \eqref{eq:2} is the constitutive law of the homogeneous, prestressed material ($\strain$: local strain tensor). Eq. \eqref{eq:3} expresses the fact that $\strain$ is /geometrically compatible/: it is the symmetric part of the gradient of the displacement $\displ$. Finally, Eq. \eqref{eq:4} specifies the boundary conditions.
