...
Maybe a quick reminder would be useful, here. $\stiffness - \stiffness_0$ is said to be positive, semi-definite, if $\tens{\varpi}:\stiffness:\tens{\varpi}\geq\tens{\varpi}:\stiffness_0:\tens{\varpi}$ for all $\tens{\varpi}$. For isotropic heterogeneous (resp. reference) materials with bulk modulus $\kappa$ (resp. $\kappa_0$) and shear modulus $\shearmod$ (resp. $\shearmod_0$), this is equivalent to
