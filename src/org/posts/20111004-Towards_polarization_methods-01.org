...
\begin{align}
    \label{eq:1}
    \nabla_\vec x\cdot\tens\sigma&=\vec0&&(\vec x\in\domain),\\
    \label{eq:2}
    \tens\sigma(\vec x)&=\stiffness(\vec x):\strain(\vec x)&&(\vec x\in\domain),\\
    \label{eq:3}
...
\begin{equation*}
\tens\sigma=\volavg{\tens\sigma}=\stiffness^\text{eff}:\Strain,
\end{equation*}

where $\tens\sigma$ is the macroscopic stress.
...
\begin{equation*}
\strain(\vec x)=\Strain,\qquad\tens\sigma(\vec x)=\stiffness_0:\Strain.
\end{equation*}
...
\begin{equation}
\label{eq:5}
\tens\sigma(\vec x)=\stiffness(\vec x):\strain(\vec x)=\stiffness_0:\strain(\vec x)+\left[\stiffness(\vec x)-\stiffness_0\right]:\strain(\vec x).
\end{equation}
...
\begin{equation}
\label{eq:7}
\tens\sigma(\vec x)=\stiffness_0:\strain(\vec x)+\polarization(\vec x).
\end{equation}
...
\begin{align*}
\nabla_\vec x\cdot\tens\sigma&=\vec0&&(\vec x\in\domain),\\
\tens\sigma(\vec x)&=\stiffness_0:\strain(\vec x)+\polarization(\vec x)&&(\vec x\in\domain),\\
\strain(\vec x)&=\nabla_\vec x^\text s\displ&&(\vec x\in\domain),\\
\displ(\vec x)&=\Strain\cdot\vec x&&(\vec x\in\partial\domain),
\end{align*}
