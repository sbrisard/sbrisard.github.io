...
We again consider a heterogeneous material, with local stiffness $\stiffness$, located in domain $\Omega\subset\reals^d$. As will be discussed elsewhere, the local problem of micromechanics reads
...
Eqs. \eqref{eq:1} -- \eqref{eq:4} would be trivial if we had $\stiffness(\vec x)=\stiffness_0$ for any $\vec x\in\Omega$ ($\stiffness_0$ is a /constant/ stiffness tensor). Indeed, the solution would then be
...
\begin{align*}
\nabla_\vec x\cdot\stress&=\vec0&&(\vec x\in\Omega),\\
\stress(\vec x)&=\stiffness_0:\strain(\vec x)+\polarization(\vec x)&&(\vec x\in\Omega),\\
\strain(\vec x)&=\nabla_\vec x^\text s\vec{u}&&(\vec x\in\Omega),\\
\vec{u}(\vec x)&=\Strain\cdot\vec x&&(\vec x\in\partial\Omega),
\end{align*}

which characterize the elastic equilibrium of a /homogeneous/, /prestressed/ linear elastic material $\stiffness_0$. From [[file:./20110911-The_Green_operator_for_strains-01.org][this series]], the solution to the above problem is known (at least, formally)

\begin{equation}
\label{eq:8}
\strain(\vec x)=\Strain-(\tens{\Gamma}_0\circledast\polarization)(\vec x),
\end{equation}

where the Green operator for strains $\tens{\Gamma}_0$ associated with $\stiffness_0$ has been introduced. We now need to remember that the polarization $\polarization$ is /not/ known. Rather, it is given by Eq. \eqref{eq:6}. Upon introduction into Eq. \eqref{eq:8}, we get the so-called Lippmann--Schwinger equation

\begin{equation}
\label{eq:9}
\strain(\vec x)+\left\{\tens{\Gamma}_0\circledast\left[\left(\stiffness-\stiffness_0\right):\strain\right]\right\}(\vec x)=\Strain.
\end{equation}

This equation was first introduced by [[https://doi.org/10.1002/pssb.2220550241][Zeller and Dederichs (1973)]]. In the next instalments of this series, it will be more convenient to chose the polarization $\polarization$, rather that the strain $\strain$ as principal unknown. This leads to the equivalent integral equation

\begin{equation}
\label{eq:10}
\left[\stiffness(\vec x)-\stiffness_0\right]^{-1}:\polarization(\vec x)+\left(\tens{\Gamma}_0\circledast\polarization\right)(\vec x)=\Strain.
\end{equation}
