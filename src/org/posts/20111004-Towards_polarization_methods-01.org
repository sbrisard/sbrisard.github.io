...
\begin{align}
    ...
    \label{eq:2}
    \stress(\vec x)&=\tens{C}(\vec x):\strain(\vec x)&&(\vec x\in\Omega),\\
    \label{eq:3}
...
where $\Strain$ is the macroscopic (imposed) strain. The solution to this problem defines the effective elastic properties $\tens{C}^\text{eff}$, since, by definition

\begin{equation*}
\Stress=\volavg{\stress}=\tens{C}^\text{eff}:\Strain,
\end{equation*}
...
\begin{equation*}
\strain(\vec x)=\Strain,\qquad\stress(\vec x)=\tens{C}_0:\Strain.
\end{equation*}
...
\begin{equation}
\label{eq:5}
\stress(\vec x)=\tens{C}(\vec x):\strain(\vec x)=\tens{C}_0:\strain(\vec x)+\left[\tens{C}(\vec x)-\tens{C}_0\right]:\strain(\vec x).
\end{equation}
...
\begin{equation}
\label{eq:6}
\tens{\tau}(\vec x)=\left[\tens{C}(\vec x)-\tens{C}_0\right]:\strain(\vec x),
\end{equation}
...
\begin{equation}
\label{eq:7}
\stress(\vec x)=\tens{C}_0:\strain(\vec x)+\tens{\tau}(\vec x).
\end{equation}
...
\begin{align*}
\nabla_\vec x\cdot\stress&=\vec0&&(\vec x\in\Omega),\\
\stress(\vec x)&=\tens{C}_0:\strain(\vec x)+\tens{\tau}(\vec x)&&(\vec x\in\Omega),\\
\strain(\vec x)&=\nabla_\vec x^\text s\vec{u}&&(\vec x\in\Omega),\\
\vec{u}(\vec x)&=\Strain\cdot\vec x&&(\vec x\in\partial\Omega),
\end{align*}
...
\begin{equation}
\label{eq:9}
\strain(\vec x)+\left\{\tens{\Gamma}_0\circledast\left[\left(\tens{C}-\tens{C}_0\right):\strain\right]\right\}(\vec x)=\Strain.
\end{equation}
...
\begin{equation}
\label{eq:10}
\left[\tens{C}(\vec x)-\tens{C}_0\right]^{-1}:\tens{\tau}(\vec x)+\left(\tens{\Gamma}_0\circledast\tens{\tau}\right)(\vec x)=\Strain.
\end{equation}
